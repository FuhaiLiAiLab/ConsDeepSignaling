...
class Output():
    ...
    def combine_non_output(self, folder_name, model_name):
        print('\n------- NOT DENSELY CONNECTED DNN PREDICTION RESULTS -------')
        ...
        merged_pred_df.to_csv('./datainfo/result/5-fold/consDeepSignaling-prediction.csv', index = False, header = True)
    ...
    def combine_dnn_output(self, folder_name, model_name):
        ...
        merged_pred_df = merged_pred_df.reset_index(drop = True)
        print(merged_pred_df)
        merged_pred_df.to_csv('./datainfo/result/5-fold/DNN-prediction.csv', index = False, header = True)
        # CALCULATE THE MSE AND PEARSON CORRELATION FOR TEST DATA SETS
        test_auc_list = list(merged_pred_df['AUC'])
        test_auc = np.array(test_auc_list)
        test_pred_list = list(merged_pred_df['Pred Score'])
...
if __name__ == "__main__":
    ...
    # OUTPUT NOT DENSELY CONNECTED DNN COMBINED PREDICTION
    model_name = 'epoch_99_No'
    Output(dir_opt).combine_non_output(folder_name, model_name)

    # OUTPUT DNN COMBINED PREDICTION
    model_name = 'epoch_100_DNN'
    Output(dir_opt).combine_dnn_output(folder_name, model_name)