...
from gen_matrix_path_dnn import GenMatrix
from load_data_path_dnn import LoadData
from keras_rand_nn_path_dnn import RandNN, RunRandNN

# BUILD DECOMPOSED MODEL
def build_rand_nn(matrixB, num_gene, num_pathway, layer1, layer2):
    input_model, pathway_model, model = RandNN().keras_rand_nn(matrixB, num_gene, num_pathway, layer1, layer2)
    return input_model, pathway_model, model

# RUN MODEL (AUTO UPDATE WEIGHT)
def run_rand_nn(model, dir_opt, matrixB, input_num, epoch, batch_size, verbose, learning_rate, end_epoch):
...
def manual_test_rand_nn(matrixB, num_gene, num_pathway, layer1, layer2, path, dir_opt, learning_rate):
    # MANUAL REBUILD THE MODEL
    input_model, pathway_model, model = build_rand_nn(matrixB, num_gene, num_pathway, layer1, layer2)
...
# CONTINUE RUN MODEL
def continue_run_rand_nn(matrixB, num_gene, num_pathway, layer1, layer2, path,
    ...
    for i in range(num_layer):
        model.get_layer(index = i).set_weights(layer_list[i])
    # RUN MODEL (AUTO UPDATE WEIGHT)    
    model, history, path = run_rand_nn(model, dir_opt, matrixB, input_num, epoch, batch_size, verbose, learning_rate, end_epoch)
    return model, history, path

#################################################################################################

def initial_run_dnn(matrixB, num_gene, num_pathway, layer1, layer2,
...
def schedule_cont_run_dnn(matrixB, num_gene, num_pathway, layer1, layer2, path,
    ...
    model, history, path = continue_run_rand_nn(matrixB, num_gene, num_pathway, layer1, layer2, path,
                dir_opt, input_num, epoch, batch_size, verbose, learning_rate, end_epoch)
    auto_test_rand_nn(model, dir_opt, verbose, path)


def learn_schedule_dnn(matrixB, num_gene, num_pathway, layer1, layer2,
    ...
    initial_run_dnn(matrixB, num_gene, num_pathway, layer1, layer2,
                dir_opt, input_num, epoch, batch_size, verbose, learning_rate)
    # CONTINUE SCHEDULING
    count = 1
    ...
    for start_epoch in schedule_start_epoch_list:
        ...
        count = count + 1
        path = '.' + dir_opt + '/result/epoch_%d' % (start_epoch)
        schedule_cont_run_dnn(matrixB, num_gene, num_pathway, layer1, layer2, path,
                    dir_opt, input_num, epoch, batch_size, verbose, learning_rate, start_epoch)
...
if __name__ == "__main__":
    ...
    # learning_rate = 0.001
    # manual_test_rand_nn(matrixB, num_gene, num_pathway, layer1, layer2, path, dir_opt, learning_rate)
    ...
    # manual_test_rand_nn(matrixB, num_gene, num_pathway, layer1, layer2, path, dir_opt)
    ...
    batch_size = 256
    verbose = 0
    learn_schedule_dnn(matrixB, num_gene, num_pathway, layer1, layer2, dir_opt, input_num, batch_size, verbose)